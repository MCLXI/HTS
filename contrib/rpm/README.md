...
The RPM spec file provided here is for HTS-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://HTS.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[HTS git repository](https://github.com/HTS/HTS) but are not part of
...
Currently only `Source10` is used, for the example `HTS.conf` file.
...
in the HTS git repository. At present only an SVG version of the HTS
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the HTS
...
needed when 0.12.1 is released, a proper fix is already in the HTS git
master branch.
...
idea to build HTS Core against a static version of that library compiled
...
The `HTS.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `HTS-server` package. At this point, most RPM based
...
The `HTS-server` package should have SELinux support. How to properly do
...
HTS-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/HTS/). He has been
packaging HTS for Fedora at least since 2012.
...
4. I renamed the `HTS` package that contains the Qt GUI to `HTS-core` as
that appears to be how the general population refers to it, in contrast to
`HTS-xt` or `HTS-classic`. I wanted to make sure the general population
