...
###   This script attempts to download the signature file SHA256SUMS.asc from HTS.org
...
RCSUBDIR="test/"
BASEDIR="https://HTS.org/bin/"
VERSIONPREFIX="HTS-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'HTS-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the HTS Core binary release signing key installed?"
   fi
